# IMP-0003 Candidate-v4 — Fresh Independent Post-Implementation Conformance Review
...
**Implementation candidate ref:** `imp-0003-slice3-conformance-candidate-v4`  
**Exact candidate SHA:** `b5e5409ebcaec856a74ab08a7d120a67f6078e36`  
**Failed parent candidate:** `imp-0003-slice3-conformance-candidate-v3` @ `35c4fb83bdac15e09c9e89e5dad67a5b6fae7048`  
...
**Exact-head CI:** run `35945067153` — PASS  
**Evidence artifact:** `10786760862`, `sfl-slice1-slice2-slice3-acceptance-evidence`, digest `sha256:f89b3c09fd386b1efb8c1678b4f49eeb503c1b96a73ce105b71cdfcf13a77d21`  
**Required return path:** `research/technical/IMP-0003/IMP-0003_INDEPENDENT_CONFORMANCE_REVIEW_V4.md`

## 1. Purpose

Independently determine whether exact candidate `b5e5409ebcaec856a74ab08a7d120a67f6078e36` conforms to:
...
For stronger independence, do not use any candidate-v1/v2/v3 conformance reviewer as this candidate-v4 reviewer.
...
- verify `imp-0003-slice3-conformance-candidate-v4` resolves exactly to `b5e5409ebcaec856a74ab08a7d120a67f6078e36`;
...
Independently challenge the checker across formation, participation, continuation, lineage, lifecycle publication, and exact retained Recognition evidence identity.

Detached corruptions must reach the checker itself. At minimum test:

- materially false warrant/event/support/tie/Recognition references previously targeted in v2/v3;
- nonexistent or mismatched Household lifecycle event IDs;
- false lifecycle Cycle/ReactionIndex;
- lifecycle event belonging to another Household or wrong published state;
- fabricated Recognition `EvidenceId` with otherwise identical proposition/provenance;
- same-cycle directly acquired and communicated evidence identities.

Valid Active, Inactive, reactivated Active, Dissolved, stale-Recognition, same-cycle handoff and same-cycle communication controls must still pass.

`HouseholdOracle` and related independent checkers must not call production classifiers as expected-answer oracles. Observer-only acquisition receipts, if used, must be non-authoritative evidence about the exact fact identity actually retained and must not alter behavior, IDs, policy inputs or semantic outcomes.
...
21. Household-connected compatible ordinary loans among current/relevant Household actors commute without fallback across ID/input/world permutations;
22. other compatible Household-connected allocation cases (at least favour or marriage) do not acquire false fallback from allocation identities;
23. detached lifecycle event/time corruptions are rejected while valid lifecycle controls pass;
24. fabricated retained and same-cycle Recognition EvidenceIds are rejected while legitimate stale/direct/communicated bases pass.
...
- run `35945067153` belongs to exact candidate SHA;
- run conclusion is success;
- artifact `10786760862` exists, is unexpired, and has digest:
  `sha256:f89b3c09fd386b1efb8c1678b4f49eeb503c1b96a73ce105b71cdfcf13a77d21`;
...
Also verify the candidate-v4 evidence artifact records `ImplementationHead=b5e5409ebcaec856a74ab08a7d120a67f6078e36`, `WorkingTreeDirty=false`, and the frozen 96/11/2 Slice-3 classifications plus 128+46 inherited REQUIRED sets. Verify the repair-v4 control evidence independently rather than accepting its counts as proof.
...
`review/imp-0003-conformance-v4-independent-return`

from exact candidate SHA:

`b5e5409ebcaec856a74ab08a7d120a67f6078e36`

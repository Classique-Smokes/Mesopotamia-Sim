...
3. the current Slice-1 CallFavor action applicability derivation is complete;
4. a fresh independent reviewer has checked this manifest against the accepted semantic/verification sources;
5. every row has one explicit status and rationale.
...
- Manifest author: Master Architect or separately commissioned verification author, not IMP-0001 production implementer.
...
- After freeze, IMP-0001 may not edit/reclassify this manifest.

## K2. IMP translation

The implementation agent may choose ordinary test framework/helper/project layout and map executable test/report identifiers to `AcceptanceId`s.
...
## K4. Independent post-implementation conformance review
...
IMP-0001 may be `VERIFIED COMPLETE` only when:
...
- all deliberate deferrals remain visible and match this frozen manifest.

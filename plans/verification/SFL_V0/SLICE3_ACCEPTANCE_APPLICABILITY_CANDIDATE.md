# SFL v0 Slice 3 — Acceptance / Applicability Authority Candidate v2

**Status:** CANDIDATE / PRE-CODE RE-REVIEW REQUIRED / NOT FROZEN  
**Candidate version:** `SFL-V0-S3-ACCEPTANCE-CANDIDATE-v2`  
...
**Previous candidate:** v1 blob `e73fab00d779985fc0f96613d14503ed8144fb5d` — pre-code review **BLOCK — VERIFICATION-AUTHORITY REPAIR**  
**Previous review:** `research/technical/SFL_V0_SLICE3_ACCEPTANCE_PRECODE_REVIEW_V1.md`
...
### Canonical positive formation witness

Slice-3 completion must include the canonical `VS-SFL-001`-shaped three-founder formation witness.

Use founders A/B/C with:

- all three co-resident at the same dwelling;
- a connected durable-tie structure spanning all three (for example A↔B and B↔C sibling links);
- one qualifying committed support event on cycle N and a second qualifying committed support event on a later distinct cycle M;
- actor-specific evidence acquisition/communication such that **all three** independently hold sufficient uncontested evidence for shared residence, connected durable ties, and both support events;
- `CandidateRecognition(C)` by all three for the same single bounded candidate referent;
- automatic `FormationWarrant → Household H → three founding SustainingParticipant associations → direct founder H Recognition`;
- H becoming Active only at stable closure;
- independently reconstructable FormationWarrant provenance;
- earliest retained sustaining-organization evidence time and formation/reification time both recorded, correct, and observably distinct.

The two-person formation case may be retained as smoke/debugging evidence but cannot substitute for this canonical positive completion witness.
...
| `S3-002-FORMATION-LOWER-LEVEL` | REQUIRED | Canonical positive formation uses the pinned VS-SFL-001 three-founder A/B/C shape: one connected durable-tie structure spanning all three, two qualifying support commits on distinct cycles, sufficient actor-specific evidence, CandidateRecognition by all three for one bounded candidate referent, automatic FormationWarrant→H→three founding SustainingParticipants→direct founder H Recognition, and Active publication only at stable closure. |
...
| `S3-006-FORMATION-WARRANT-PROVENANCE` | REQUIRED | Warrant references founders, dwelling, durable ties, qualifying events, recognition evidence, earliest sustaining evidence, formation time and rules/config version; earliest retained sustaining-evidence time and formation/reification time are both correct and observably distinct. |
...
| `S3-065-LIFECYCLE-EVIDENCE-UPDATES-H-UNDERSTANDING` | REQUIRED | Slice 3 executes both mandatory lifecycle/continuity Recognition subcases S3-065A and S3-065B through accepted direct-participation/communication routes only. |
| `S3-065A-CONTINUATION-EVIDENCE-REFRESH` | REQUIRED | Outsider X first holds older Recognized(H exists/continues), remains stale across an unwitnessed continuing-H transition, then receives newer event-backed same-H continuation evidence from a direct participant and ends with current Recognized(H exists/continues) provenance. |
| `S3-065B-DISSOLUTION-EVIDENCE-NO-SAFE-RELIANCE` | REQUIRED | Outsider X first holds older positive continuation Recognition, remains stale through unwitnessed final-bearer dissolution, then receives decisive event-backed participation-end/dissolution evidence through ordinary communication; X's factual/provenance basis updates and the stale positive continuation basis is no longer safely relied upon. No specific negative Recognition enum is required. |
...
| `S3-116-IRRELEVANT-ATTRIBUTE-PERTURBATION` | REQUIRED | On representative formation/continuity/lineage histories, vary one explicitly declared semantically irrelevant attribute while holding authoritative causal inputs fixed; the bounded semantic result remains unchanged under O3-META. |
...
Pre-code review v1 closed the semantic question and required the following exact bounded pair. Candidate-v2 makes both omission-detectable.

### RG-02A — current continuation

1. outsider X holds older accepted `Recognized(H exists/continues)`;
2. H undergoes a valid continuing participation/continuity transition without X as participant or recipient;
3. X remains stale rather than passively synchronizing;
4. a direct participant later communicates newer event-backed evidence establishing the same non-Dissolved H continues;
5. X updates to current `Recognized(H exists/continues)` with provenance to that newer accepted evidence.

### RG-02B — decisive dissolution

1. outsider X holds older positive continuation Recognition for H;
2. H's final bearer ends participation and H becomes Dissolved without X as participant or recipient;
3. X remains stale until accepted evidence reaches X;
4. a direct participant who holds the decisive participation-end/dissolution evidence communicates it through the ordinary Slice-2 communication path;
5. X's factual/provenance basis updates;
6. X may no longer safely rely on the stale positive continuation basis as current;
7. **no mandatory negative Recognition enum** is asserted.

Both subcases:

- use no generic nonparticipant observation;
- use no head/authority proposition;
- preserve underlying event/provenance ordering;
- do not invent a new Recognition meaning.
...
- explicit closure of v1 repairs R1–R4.
...
Candidate-v2 requires fresh independent pre-code re-review.

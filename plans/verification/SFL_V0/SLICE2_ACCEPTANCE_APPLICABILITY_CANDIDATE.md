# SFL v0 Slice 2 — Acceptance / Applicability Authority Candidate v2

**Status:** CANDIDATE / PRE-CODE RE-REVIEW REQUIRED / NOT FROZEN  
**Candidate version:** `SFL-V0-S2-ACCEPTANCE-CANDIDATE-v2`  
...
**Previous candidate:** v1 blob `cc9ec48fc844645bcf8b7c44cc3660074b512245` — pre-code review **BLOCK**  
**Previous review:** `research/technical/SFL_V0_SLICE2_ACCEPTANCE_PRECODE_REVIEW_V1.md`
...
This candidate repairs the v1 review defects. It may instantiate accepted semantics with bounded laboratory witnesses; it may not create new proposition meaning, perception rules, trust/rumor/reputation, household/head semantics, checkpoint semantics, or richer agency.
...
| AcceptanceId | Status | Obligation |
|---|---|---|
| `S2-020-COMMUNICATION-ACTION` | REQUIRED | CommunicateClaim/Inform executes through the ordinary personal-action pipeline. |
...
| `S2-024-ORDINARY-COMMUNICATION-INITIATIVE-COST` | REQUIRED | A successful ordinary pinned-policy CommunicateClaim/Inform initiation consumes the sender's personal initiative and prevents a second ordinary voluntary personal initiation that cycle. |
| `S2-025-NO-FREE-BROADCAST` | REQUIRED | Only declared intended recipient(s) acquire the communication; no global propagation occurs. |
| `S2-026-RECEIPT-NO-REACTIVATION-OR-RESPONSE` | REQUIRED | Receipt alone creates neither fresh same-cycle personal activation nor a ResponseDecisionContext. |
...
|---|---|
| `S1-098-COMMUNICATION` | S2-020 through S2-027, including factual and Recognition communication |
| `S1-META-08` | S2-032 |
...
| S2-020 | F-COMM-FACT | O-PUBLIC | direct epistemic mutation instead of action pipeline |
...
| S2-024 | F-GATE | O-PUBLIC | production initiative counter helper as sole oracle |
| S2-025 | F-COMM-FACT | O-META/O-PUBLIC | global enumeration inferred from one sampled actor |
| S2-026 | F-COMM-FACT | O-PUBLIC | response/personal scheduler helper as expected answer |
...
### S2-025 population scope

The scenario's entire declared population is the bounded domain. Assert intended recipient(s) acquire and every other fixture actor does not. Do not infer a global broadcast claim from sampling one nonrecipient.
...
Start with live Recognized CandidateRecognition(C). In paired variants:

- perturb an unrelated directed Attitude into a negative band; and
- optionally exercise an otherwise independent ordinary voluntary Decline.

Hold C and its evidence fixed. Recognition(C) must remain unchanged solely under those perturbations.
...
## 17. Pre-code review requirements for candidate v2

A **different fresh regular review chat** must re-derive from accepted sources and return PASS/BLOCK.

It must specifically verify:

- v1 BLOCK repair completeness;
- exact RG-01 witness semantic support;
- no hidden negative Recognition proposition or new evidence-strength rule;
- Recognition communication omission-detectability;
- Recognition-vs-attitude/compliance coverage;
- all three accepted precedence subcases;
- exact Slice-1 regression bindings;
- per-row fixture/oracle mappings;
- no Household/head/checkpoint pull-forward;
- broad/global wording bounded by explicit census/metamorphic domain;
- no new Director-level semantic decision.

## 18. Freeze mechanics

Only after independent PASS:

- promote reviewed bytes as `SFL-V0-S2-ACCEPTANCE-v1`;
- record reviewed candidate commit/blob;
- create immutable freeze ref;
- record final frozen commit/content identity externally in a freeze record;
- any semantic/applicability edit after freeze requires a new version/review.
...
## 20. Current release judgment

**NO RELEASE TO IMPLEMENTATION YET.**

Candidate v2 must receive fresh independent pre-code review and then be frozen unchanged if it passes.

...
For v2, an external successor freeze record must be created only after fresh independent whole-candidate review returns PASS; that record must pin the exact v2 candidate commit/blob, final review report, successor frozen manifest/ref, census and inherited set identities.

## 10. Candidate implementation handoff rule

This manifest is a **successor candidate**, not frozen authority.
...
5. run fresh independent adaptation review against the reassessed exact candidate;
6. only after adaptation closure may the Director implementation block be closed and the bounded Slice-4 implementation packet be issued.

Future implementation agents may not edit, weaken, delete, reclassify, or replace frozen v1 or the eventual frozen successor acceptance authority.

Complete implementation evidence must report every successor REQUIRED row and all 270 inherited frozen REQUIRED rows individually. If implementation requires inventing a consequential result not uniquely determined by accepted authority, stop and escalate rather than changing acceptance authority or guessing in code.

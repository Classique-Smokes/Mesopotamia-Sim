...
| LATER-04 | Authority-destroying transition versus already accepted authority-dependent action in same cycle | **CLOSED / RESOLVED BY DEC-0010** | Director selected authority-use before authority destruction. Promoted into SPEC §9.2 and canonical verification. | Slice-4 acceptance may now bind deterministic last-act-before-destruction witnesses; no technical-ID authority. |
| LATER-05 | Mediated-marriage nested response / authority topology beyond the current accepted route wording | **CLOSED / RESOLVED BY ACCEPTED AUTHORITY** | Slice-4 reconciliation found accepted SPEC §§4.2, 5.1, 8.4 plus `VS-SFL-104` already fix the bounded route: groom initiates; current head is the role-scoped responder; head acceptance is sufficient; no extra bride response; female-head self-mediation is permitted. | No semantic amendment required; do not reopen absent a contradictory implementation need. |

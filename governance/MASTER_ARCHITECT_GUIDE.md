...
7. **Promote deliberately, losslessly, and discoverably** — after explicit approval, inventory the approved operative commitments before compression, then write the concise canonical form and reconcile every inventory item as preserved, explicitly superseded/deferred, or recorded as an open erratum. Remove rationale/examples when useful; never remove operative conditions, edge cases, exclusions, or delegated locks merely to shorten the record. If the promoted artifact is intended to govern or inform future AI work, update the relevant navigation/index surfaces in the same promotion step so a fresh agent can discover it from the repository entry path without chat or Git-history archaeology. Do not promote adjacent conversation automatically.

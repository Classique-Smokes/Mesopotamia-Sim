...
### 3.0 Endpoint identity / diagonal admissibility

Endpoint identity is owned by each typed semantic meaning. There is no universal rule that every relation endpoint or every person-valued action role must be distinct.

For typed relations/claims whose endpoint identity domains can overlap, the owning semantics classify whether the diagonal is:

- allowed;
- not enabled in current semantics while representational room is preserved; or
- inapplicable/structurally excluded by endpoint typing or accepted meaning.

Generic relation storage/index/schema machinery must not impose one project-wide endpoint-inequality rule. Type-specific validation may enforce the owning relation's accepted classification.

This relation-state classification does not itself authorize self-addressed interactions. Participant-position aliasing inside an action/proposal is a separate semantic question.

Current bounded-v0 classifications:

- **Attitude:** diagonal allowed. `Attitude(P,P)` is valid state using the ordinary [-100,+100] domain and ordinary maintenance/decay. Its existence creates no new current-v0 action or scorer meaning by itself.
- **Debt:** diagonal not enabled. Current Debt requires distinct creditor and debtor PersonIds.
- **Favour:** diagonal not enabled. Current Favour requires distinct holder and debtor PersonIds.
- **Kinship:** diagonal structurally excluded for current parent/child and sibling meanings.
- **Marriage:** diagonal structurally excluded for the current bounded groom/bride meaning.
- **Residence:** stored relation is Person -> Dwelling, so same-domain diagonal identity is not applicable; its proposal semantics separately require another person as counterpart.

A future typed relation/claim must classify this question when conceptualized rather than inheriting a generic technical default. See DEC-0015.
...
The directed endpoints may be identical: `Attitude(P,P)` is valid bounded-v0 state. It follows the same numeric bounds and maintenance/decay rules as any other attitude edge. This does not by itself authorize P as both parties of an interaction whose own participant-alias semantics require distinct people, and it creates no additional current-v0 psychological/scoring interpretation.
...
Kinship is initialized rather than generated by reproduction. Parent/child and sibling endpoints must be distinct PersonIds; diagonal kinship is not a valid current relation.
...
Debt is a quantitative grain claim. Current v0 Debt requires distinct creditor and debtor PersonIds; a diagonal self-debt is not a valid current claim or initialization state. This is a Debt-specific semantic rule, not a generic relation-container constraint.
...
A favour is one qualitative action-oriented claim. Current v0 Favour requires distinct holder and debtor PersonIds; a diagonal self-favour is not a valid current claim or initialization state. This is a Favour-specific semantic rule, not a generic relation-container constraint.
...
- current v0 `CallFavor` requires holder and called actor to be distinct PersonIds; a self-call is categorically invalid before called-actor response activation;
...
For the current bounded-v0 interpersonal transfer proposal forms, initiator/authorizer and named counterparty must be distinct PersonIds. This applies to `OfferGift`, `RequestGiftOrHelp`, `OfferLoan`, `RequestLoan`, explicit benefit-for-favour, and relationship-mediated reciprocal help. A self-counterparty term is categorically invalid before response activation; it creates no valid material transfer, social success/refusal effect, or §6.7 failed-attempt outcome. This is meaning-specific and does not prohibit valid same-person reuse across separate actions, responses, claims, or nested compositions.

Current transfer meanings:
...
- groom and bride are distinct PersonIds in current bounded v0;
- groom initiates;
...
- 2026-09-25 — Director-approved DEC-0014 provision-reconsideration clarification: the arbitrary three-cycle rule is explicitly a crude v0 anti-repeated-solicitation proxy; a qualifying Household material-need occurrence after the latest refusal historically satisfies that context-change limb even if the need later ends, an otherwise-valid unfunded dowry request can count, and a later refusal resets the baseline.

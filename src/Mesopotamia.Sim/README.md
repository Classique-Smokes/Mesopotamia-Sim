...
The canonical personal profiles are `SCORE-VP-002/004/005/006`; response profiles are `SCORE-RP-001/002/003/004`. Their meanings are pinned by the frozen verification material. Default preferences are laboratory configuration, not historical calibration. Candidate generation and categorical/feasibility rules are shared across policies. Exact integer components and ordinal stable candidate keys determine selection, with explicit tie markers.
...
Action commits prepare a complete copied state, check arithmetic and invariants, then replace authoritative state. Direct attitude causes retain individual provenance, deduplicate, sum, and clamp once per directed pair. Debt social-due review participates in the same batch. The stable-ID ordering is technical fallback where unresolved alternatives compete; it is not a social preference. Response feasibility and higher candidate scores run before fallback.
